<?php
	...
	$redis -> mset(['name'=>'roger', 'sex'=>1, 'age'=>31]);
	

	/* auth password   			  	设置密码
	 * select db_index    		  	选择数据库
	 * 
	 * mset key1, value1, key2, value2, key3, value3 设置多个key值
	 * set key, value [EXPIRE]    	设置key值
	 * setex key expire value		设置key值，并为该值设定过期时间  如果key已经存在，命令将会替换旧的值。 
	 * setnx key value				设置key值 指定的key不存在时，为key设置指定的值，key存在返回0
	 * getset key, new_value      	设置新的key的值，并返回旧的key值
	 * 
	 * mget key1, key2, key3      	获取多个key值
	 * get key  	          	  	获取key的值
	 * getrange key, start, end   	获取key中start到end的值
	 * 
	 * del key         			  	删除key的值
	 * 
	 * move key, db_index  		  	将key移动到其他数据库
	 * exists key      			  	key是否存在
	 * 
	 * expire key      				设定过期时间
	 * persist key     				去除过期时间
	 * ttl key /pttl key 			返回过期秒数/毫秒数(返回int：剩余时间；-1：没有过期时间；-2：不存在该值)
	 * 
	 * keys *			   			列出当前数据库所有的key
	 * randomkey	       			随机当前数据库中的key
	 * flushdb			   			删除当前数据库中的key
	 * 
	 * rename key, new_key   		重命名key 【当key和new_key相同，或者key不存在时，返回一个错误。 当new_key已经存在时，RENAME命令将覆盖旧值。】
	 * renamex key, new_key  		重命名key 【修改成功时，返回1。如果new_key已经存在，返回 0 。】
	 * 
	 * type key     				获取值类型
	 */
